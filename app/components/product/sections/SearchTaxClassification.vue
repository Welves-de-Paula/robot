    ...
    <v-dialog v-model="dialog" scrollable max-width="500px">
      <v-card :title="`Selecionar ${label}`">
        ...
        <v-card-text>
          ...
          <v-list :lines="false">
            <v-list-item
              v-for="(group, i) in taxClassifications"
            ...
            >
              <template v-slot:prepend>
                <small class="mr-2">{{ group.code }}# </small>
              </template>
...
const select = async () => {
  try {
    $loading.start();
    await $http
      .index("product/tax-classifications", query.value)
      .then((response) => {
        taxClassifications.value = response.taxClassifications;
      });
  } catch (error) {
    console.log(error);
  } finally {
